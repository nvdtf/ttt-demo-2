...
- [x] T010 [US1] Wire `handleCellClick` into the cell click handlers created by `renderBoard()`, passing the cell index, in the `<script>` block of `index.html`
...
- [ ] T017 *(non-normative, probe-default from D5)* Add subtle hover background highlight on empty cells when the game is not over using a CSS `:hover` selector scoped to non-occupied, non-game-over state, in `index.html`
- [ ] T018 [FR-013] Ensure grid cells are keyboard-accessible by using `<button>` elements (or adding `tabindex` and `keydown` handlers) so users can navigate and activate cells with Tab and Enter, in `index.html`
